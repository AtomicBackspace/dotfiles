...
function on_press()
  ...
  if target_ar < aspect * 0.99 then
    -- Reduce width
    new_w = (height * target_ar) /
        par        -- New width is the width multiple by the aspect ratio, adjusted for the PAR (pixel aspect ratio) incase it's not 1:1
    new_h = height -- Height stays the same since we only ever crop one axis in this script
    new_x = (width - new_w) /
        2          -- Width - new width will equal the total space cropped, since its evenly cropped from both sides the offset needs to be halved
  ...
  elseif target_ar > aspect * 1.01 then
    ...
    new_h = (width * (1 / target_ar)) *
        par                      -- See new_w above, need to adjust for PAR but it's in the reverse direction
